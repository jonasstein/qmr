...
void printhelp(){
  ...
  std::cout << "Usage: mkhistogram <ChDet> <ChSync> <ChSemaphore> <ChMonitor> <filename> <bins> <mode>" << std::endl;
...
int main(int argc, char *argv[]){
  ...
  if (argc != 8)
  {
    std::cerr << "Error wrong number of arguments. Expected 7, got " << argc-1 << ". Stopped." << std::endl;
    printhelp();
    exit(3);
  }
  ...
  std::cerr << "ChDet:" << ArgChDet << " ChSync:" << ArgChSync << " ChSemaphore:" << ArgChSemaphore << " ChMonitor:" << ArgChMonitor << std::endl;
    ...
    if (ArgMode==HISTOGRAMMODE){
      ...
      while (!ifs.eof()){
        ...
        CURRENTts-=StartOffsetts;
          ...
          if ((TrigID==7)&&(DataID==ArgChSemaphore)){ //found a SEMAPHORE event (new histogram/new scan)
            if (FirstPrintOut){
